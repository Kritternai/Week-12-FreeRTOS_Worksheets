...
// เลือกโหมดการทดสอบ: 0 = Cooperative, 1 = Preemptive
#define TEST_MODE_COOPERATIVE 0
#define TEST_MODE_PREEMPTIVE 1

// เปลี่ยนค่านี้เพื่อสลับระหว่าง Cooperative และ Preemptive
#define TEST_MODE TEST_MODE_PREEMPTIVE

// ===== Cooperative Multitasking Section =====
#if TEST_MODE == TEST_MODE_COOPERATIVE
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "LED1 (GPIO%d): Task 1", LED1_PIN);
    ESP_LOGI(TAG, "LED2 (GPIO%d): Task 2", LED2_PIN);
    ESP_LOGI(TAG, "LED3 (GPIO%d): Emergency response", LED3_PIN);
    
    cooperative_scheduler();
}

// ===== Preemptive Multitasking Section =====
#elif TEST_MODE == TEST_MODE_PREEMPTIVE

static const char *TAG = "PREEMPTIVE";
...
void preemptive_task1(void *pvParameters)
{
    uint32_t count = 0;
    while (1) {
        ESP_LOGI(TAG, "Preempt Task1: %d", count++);
...
void preemptive_task2(void *pvParameters)
{
    uint32_t count = 0;
    while (1) {
        ESP_LOGI(TAG, "Preempt Task2: %d", count++);
...
void preemptive_emergency_task(void *pvParameters)
{
    while (1) {
        ...
        if (gpio_get_level(BUTTON_PIN) == 0 && !preempt_emergency) {
            ...
            ESP_LOGW(TAG, "IMMEDIATE EMERGENCY! Response: %d ms (Max: %d ms)", 
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "=== Preemptive Multitasking Demo ===");
    ESP_LOGI(TAG, "RTOS will preempt tasks automatically");
    ESP_LOGI(TAG, "Press button (GPIO0) to test emergency response");
    ESP_LOGI(TAG, "LED1 (GPIO%d): Task 1 (Priority 2)", LED1_PIN);
    ESP_LOGI(TAG, "LED2 (GPIO%d): Task 2 (Priority 1)", LED2_PIN);
    ESP_LOGI(TAG, "LED3 (GPIO%d): Emergency (Priority 5 - HIGHEST)", LED3_PIN);
