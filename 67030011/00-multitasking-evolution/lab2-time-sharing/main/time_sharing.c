...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "LED1 (GPIO%d): Sensor task (light work)", LED1_PIN);
    ESP_LOGI(TAG, "LED2 (GPIO%d): Processing task (heavy work)", LED2_PIN);
    ESP_LOGI(TAG, "LED3 (GPIO%d): Actuator task (medium work)", LED3_PIN);
    ESP_LOGI(TAG, "LED4 (GPIO%d): Display task (quick work)", LED4_PIN);
    ...
    // Run normal time-sharing for a while
    ESP_LOGI(TAG, "\n=== Starting Normal Time-Sharing ===");
    for (int i = 0; i < 100; i++) {
        manual_scheduler();
    ...
    // Now run variable time slice experiment
    vTaskDelay(pdMS_TO_TICKS(2000));
    variable_time_slice_experiment();

    ESP_LOGI(TAG, "\n=== Experiment Complete ===");
    ESP_LOGI(TAG, "System will continue running in loop...");
